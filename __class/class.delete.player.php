<?php 
	...
	*/
	class deletePlayer extends dbconnect
	{
		protected $plug;
		protected $id;
		...
		{
			# write delete query
			$deleteQuery = "DELETE FROM players WHERE id = '".$this->id."' ";

			// validate if post has been deleted 
			if (mysqli_query($this->plug, $deleteQuery) === TRUE) {
				# code...
				echo("<div class='alert alert-success'><button class='close' data-dismiss='alert' id='msg-btn' onclick='return sessionRemove();'>&times;</button><h5 style='color:green'>Player deleted successfully.</h5></div>");
				exit();
			}else{
